...
import { 
  ...
  ChevronLeft, ChevronRight, ChevronsLeft, ChevronsRight, Target, Stethoscope, Leaf
} from "lucide-react";
...
// ==========================================
interface ExpertEngineFormDict {
  title: string; subtitle: string; formTitle: string;
  q1: string; q1Opt1: string; q1Opt2: string; q1Opt3: string;
  q2: string; q2OptPond: string;
  q3: string; q3Opt1: string; q3Opt2: string;
  q4: string; q4Opt1: string; q4Opt2: string; q4Opt3: string;
  q5: string; q5Opt1: string; q5Opt2: string; q5Opt3: string;
  q6: string; q6Opt1: string; q6Opt2: string; q6Opt3: string; q6Opt4: string; q6Opt5: string;
  needCarpet: string; needRed: string; needShrimp: string;
  btnStart: string; idleTitle: string; idleDesc: string;
  failTitle: string; failDesc: string; successTitle: string; successDesc: string; matchCount: string;
  bestMatch: string; confidence: string; points: string; defaultReason: string;
...
export default function PlantExpertEngineV4() {
  ...
  // HITUNG ULANG OTOMATIS SAAT BAHASA BERUBAH (TANPA KEDIP/LOADING)
  useEffect(() => {
    if (results !== null && plants.length > 0 && serviceDict) {
      ...
      const aiResults = generateRecommendations(plants, answers, serviceDict); 
  ...
  }, [language, serviceDict, plants.length]); 
  ...
  const getConfidenceLabel = (key: string) => {
    if (!expertDict) return key;
    switch (key) {
      case "Excellent": return expertDict.confExcellent || "Sangat Cocok";
      case "VeryGood": return expertDict.confVeryGood || "Sangat Baik";
      case "Good": return expertDict.confGood || "Baik";
      default: return expertDict.confModerate || "Moderat";
    }
  };
  ...
  return (
    <div className="w-full h-full min-h-screen p-4 sm:p-6 md:p-8 lg:p-10">
      <div className="max-w-[1400px] mx-auto space-y-8 pb-10 text-slate-900 dark:text-slate-100">
        ...
        <div className="mb-8">
          <h1 className="text-3xl md:text-4xl font-extrabold text-emerald-600 dark:text-emerald-400 flex items-center gap-3">
            <Cpu className="h-8 w-8 md:h-10 md:w-10" /> {expertDict.title}
          </h1>
          ...
          <Card className="border-slate-200 dark:border-slate-800/80 bg-white dark:bg-slate-900/80 xl:col-span-4 h-fit shadow-xl shadow-slate-200/50 dark:shadow-none transition-colors duration-300">
            <CardContent className="p-6 md:p-8 space-y-6">
              <h3 className="text-lg font-bold border-b border-slate-200 dark:border-slate-800 pb-3 flex items-center gap-2 text-gray-900 dark:text-slate-100">
                <Filter className="h-5 w-5 text-emerald-600 dark:text-emerald-500" /> {expertDict.formTitle}
              </h3>

              <div className="space-y-5">
                <div className="space-y-2">
                  <Label className="text-slate-700 dark:text-slate-300 text-xs md:text-sm uppercase font-bold tracking-wider">{expertDict.q1}</Label>
                  <select value={experience} onChange={(e) => setExperience(e.target.value as ExperienceLevel)} className="w-full h-11 rounded-md border border-slate-300 dark:border-slate-700 bg-slate-50 dark:bg-slate-950 px-3 text-sm text-slate-900 dark:text-slate-200 focus:border-emerald-500 outline-none transition-all">
                    <option value="Pemula">{expertDict.q1Opt1}</option>
                    <option value="Menengah">{expertDict.q1Opt2}</option>
                    <option value="Mahir">{expertDict.q1Opt3}</option>
                  </select>
                </div>

                <div className="space-y-2">
                  <Label className="text-slate-700 dark:text-slate-300 text-xs md:text-sm uppercase font-bold tracking-wider">{expertDict.q2}</Label>
                  <select value={tankSize} onChange={(e) => setTankSize(e.target.value)} className="w-full h-11 rounded-md border border-slate-300 dark:border-slate-700 bg-slate-50 dark:bg-slate-950 px-3 text-sm text-slate-900 dark:text-slate-200 focus:border-emerald-500 outline-none transition-all">
                    ...
                    <option value="Pond">{expertDict.q2OptPond}</option>
                  </select>
                </div>

                <div className="space-y-2">
                  <Label className="text-slate-700 dark:text-slate-300 text-xs md:text-sm uppercase font-bold tracking-wider">{expertDict.q3}</Label>
                  <select value={co2} onChange={(e) => setCo2(e.target.value)} className="w-full h-11 rounded-md border border-slate-300 dark:border-slate-700 bg-slate-50 dark:bg-slate-950 px-3 text-sm text-slate-900 dark:text-slate-200 focus:border-emerald-500 outline-none transition-all">
                    <option value="Tanpa CO2">{expertDict.q3Opt1}</option>
                    <option value="Tinggi (Injeksi)">{expertDict.q3Opt2}</option>
                  </select>
                </div>

                <div className="space-y-2">
                  <Label className="text-slate-700 dark:text-slate-300 text-xs md:text-sm uppercase font-bold tracking-wider">{expertDict.q4}</Label>
                  <select value={light} onChange={(e) => setLight(e.target.value as LightLevel)} className="w-full h-11 rounded-md border border-slate-300 dark:border-slate-700 bg-slate-50 dark:bg-slate-950 px-3 text-sm text-slate-900 dark:text-slate-200 focus:border-emerald-500 outline-none transition-all">
                    <option value="Low">{expertDict.q4Opt1}</option>
                    <option value="Medium">{expertDict.q4Opt2}</option>
                    <option value="High">{expertDict.q4Opt3}</option>
                  </select>
                </div>

                <div className="space-y-2">
                  <Label className="text-slate-700 dark:text-slate-300 text-xs md:text-sm uppercase font-bold tracking-wider">{expertDict.q5}</Label>
                  <select value={maintenance} onChange={(e) => setMaintenance(e.target.value as MaintenanceLevel)} className="w-full h-11 rounded-md border border-slate-300 dark:border-slate-700 bg-slate-50 dark:bg-slate-950 px-3 text-sm text-slate-900 dark:text-slate-200 focus:border-emerald-500 outline-none transition-all">
                    <option value="Low">{expertDict.q5Opt1}</option>
                    <option value="Medium">{expertDict.q5Opt2}</option>
                    <option value="High">{expertDict.q5Opt3}</option>
                  </select>
                </div>

                <div className="space-y-2">
                  <Label className="text-slate-700 dark:text-slate-300 text-xs md:text-sm uppercase font-bold tracking-wider">{expertDict.q6}</Label>
                  <select value={style} onChange={(e) => setStyle(e.target.value)} className="w-full h-11 rounded-md border border-slate-300 dark:border-slate-700 bg-slate-50 dark:bg-slate-950 px-3 text-sm text-slate-900 dark:text-slate-200 focus:border-emerald-500 outline-none transition-all">
                    <option value="Bebas">{expertDict.q6Opt1}</option>
                    <option value="Nature">{expertDict.q6Opt2}</option>
                    <option value="Dutch">{expertDict.q6Opt3}</option>
                    <option value="Iwagumi">{expertDict.q6Opt4}</option>
                    <option value="Jungle">{expertDict.q6Opt5}</option>
                  </select>
                </div>

                <div className="pt-5 mt-2 border-t border-slate-200 dark:border-slate-800 space-y-4">
                  <label className="flex items-center gap-3 cursor-pointer group">
                    <input type="checkbox" checked={wantCarpet} onChange={(e) => setWantCarpet(e.target.checked)} className="h-5 w-5 accent-emerald-600 rounded cursor-pointer" />
                    <span className="text-sm md:text-base font-bold text-slate-700 dark:text-slate-300 group-hover:text-emerald-600 transition-colors">{expertDict.needCarpet}</span>
                  </label>
                  <label className="flex items-center gap-3 cursor-pointer group">
                    ...
                    <span className="text-sm md:text-base font-bold text-rose-700 dark:text-rose-400 group-hover:text-rose-600 transition-colors">{expertDict.needRed}</span>
                  </label>
                  <label className="flex items-center gap-3 cursor-pointer group">
                    <input type="checkbox" checked={shrimpTank} onChange={(e) => setShrimpTank(e.target.checked)} className="h-5 w-5 accent-emerald-600 rounded cursor-pointer" />
                    <span className="text-sm md:text-base font-bold text-slate-700 dark:text-slate-300 group-hover:text-emerald-600 transition-colors">{expertDict.needShrimp}</span>
                  </label>
                </div>
              </div>

              <Button onClick={runInferenceEngine} disabled={loading} className="w-full bg-emerald-600 hover:bg-emerald-500 text-white font-bold h-14 mt-6 text-base shadow-lg shadow-emerald-600/20 dark:shadow-emerald-900/30 transition-all active:scale-[0.98]">
                {loading ? <Loader2 className="mr-2 h-6 w-6 animate-spin" /> : expertDict.btnStart}
              </Button>
            </CardContent>
          </Card>
          ...
          <div className="xl:col-span-8 space-y-6">
            {!results ? (
              <div className="flex flex-col items-center justify-center h-full min-h-[500px] border-2 border-dashed border-slate-300 dark:border-slate-800 rounded-2xl bg-slate-50/50 dark:bg-slate-900/20 transition-colors p-8 text-center">
                ...
                <h3 className="text-2xl font-bold text-slate-800 dark:text-slate-200 mb-3">{expertDict.idleTitle}</h3>
              ...
              <div className="flex flex-col items-center justify-center min-h-[500px] border-2 border-red-200 dark:border-red-900/30 rounded-2xl bg-red-50 dark:bg-red-950/10 p-8 text-center transition-colors">
                ...
                <h3 className="text-2xl font-bold text-red-900 dark:text-red-200 mb-3">{expertDict.failTitle}</h3>
              ...
              <div className="animate-in fade-in duration-500 slide-in-from-bottom-4">
                ...
                <div className="grid gap-6 lg:grid-cols-2">
                  {displayedResults.map((plant, index) => {
                    ...
                    if (isTopMatch) {
                      return (
                        <div key={plant.id} className="lg:col-span-2 relative rounded-2xl overflow-hidden bg-white dark:bg-slate-900 border-2 border-emerald-500 shadow-xl shadow-emerald-500/10 mb-4 flex flex-col">
                          
                          <div className="bg-gradient-to-r from-emerald-600 to-teal-500 px-5 py-2.5 text-white font-black flex items-center gap-2 text-sm tracking-widest uppercase shadow-sm z-10 relative">
                            <Trophy className="h-5 w-5 text-amber-300" /> {expertDict.bestMatch}
                          </div>
                            ...
                            <div className="flex-1 flex flex-col bg-white dark:bg-slate-900 p-4 lg:p-5">
                              <div className="flex flex-col items-start gap-3 pb-4 mb-4 border-b border-slate-100 dark:border-slate-800">
                                ...
                                <div className={`inline-flex items-center self-start px-3 py-1.5 rounded-lg border ${getConfidenceColor(confidenceKey)} shadow-sm shrink-0`}>
                                  ...
                                  <span className="font-black text-sm">{plant.matchScore} {expertDict.points}</span>
                              ...
                              <div className="flex-1">
                                {plant.matchReasons && plant.matchReasons.length > 0 ? (
                                  <div className="grid gap-3 sm:grid-cols-1">
                                    {plant.matchReasons.map((reason, i) => (
                                      <div key={i} className="flex items-start bg-slate-50 dark:bg-slate-950/50 p-3 rounded-lg border border-slate-100 dark:border-slate-800/60 transition-colors">
                                        <div className="bg-emerald-100 dark:bg-emerald-900/40 p-1 rounded-full mr-3 shrink-0 mt-0.5">
                                  ...
                                  <p className="text-sm text-slate-500 italic p-3 bg-slate-50 dark:bg-slate-900/50 rounded-lg border border-slate-100 dark:border-slate-800">{expertDict.defaultReason}</p>
                                )}
                              </div>
                            </div>
                          </div>
                        ...
                        <div className="p-5 flex-1 flex flex-col">
                          <div className="flex flex-col items-start gap-2 mb-4 pb-4 border-b border-slate-100 dark:border-slate-800">
                            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider flex items-center gap-1.5">
                               <Stethoscope className="w-3.5 h-3.5 text-emerald-600 dark:text-emerald-500" />
                               {expertDict.confidence}
                            </span>
                            <div className={`inline-flex max-w-full flex-wrap items-center gap-x-1.5 gap-y-0.5 text-[10px] font-bold px-2.5 py-1.5 rounded-md border ${getConfidenceColor(confidenceKey)}`}>
                              <span className="whitespace-nowrap">{plant.matchScore} {expertDict.points}</span>
                              <span className="opacity-40">•</span>
                              <span className="leading-tight break-words">{getConfidenceLabel(confidenceKey)}</span>
                            </div>
                          </div>
                          {plant.matchReasons && plant.matchReasons.length > 0 ? (
                            <ul className="space-y-2 border-l-2 border-emerald-500/40 pl-3 py-1 flex-1">
                              {plant.matchReasons.map((reason, i) => (
                            ...
                            <p className="text-xs text-slate-500 italic pl-3 border-l-2 border-slate-200 dark:border-slate-800 py-1 flex-1">{expertDict.defaultReason}</p>
                          )}
                        </div>
                      </div>
                    );
                ...
                {totalPages > 1 && (
                  <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between border-t border-slate-200 dark:border-slate-800 pt-5 mt-6 gap-4 transition-colors">
                    <p className="text-xs sm:text-sm text-slate-600 dark:text-slate-400 text-center lg:text-left mb-1 lg:mb-0">
                      {expertDict.paginationShowing} <span className="font-bold text-gray-900 dark:text-slate-200">{startIndex + 1}</span> {expertDict.paginationTo} <span className="font-bold text-gray-900 dark:text-slate-200">{Math.min(endIndex, results.length)}</span> {expertDict.paginationOf} <span className="font-bold text-gray-900 dark:text-slate-200">{results.length}</span> {expertDict.paginationData}
                    </p>
                    ...
                    <div className="flex flex-col lg:flex-row items-center lg:items-center justify-center lg:justify-end gap-3 w-full lg:w-auto">
                        ...
                        {pageNumbers.map(num => (
                          <Button
                            ...
                            className={`h-8 w-8 p-0 text-sm font-medium transition-all shrink-0 ${
                              currentPage === num 
                                ? 'bg-emerald-600 hover:bg-emerald-500 text-white border-emerald-600 dark:border-emerald-500 shadow-md shadow-emerald-600/20 scale-105' 
                      ...
                      <div className="flex items-center justify-center gap-2 text-sm border-t lg:border-t-0 lg:border-l border-slate-300 dark:border-slate-700 pt-2.5 lg:pt-0 lg:pl-3 w-full lg:w-auto transition-colors text-slate-600 dark:text-slate-300">
                        <Input 
                          ...
                          className="w-12 h-8 text-center text-sm bg-white dark:bg-slate-950 border-slate-300 dark:border-slate-700 text-gray-900 dark:text-white [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none focus:border-emerald-500 transition-colors"
                        />
