...
import { Pill, FlaskConical, Beaker, Leaf, Bug, AlertTriangle, ShieldCheck, Clock, Activity, Loader2, Search, Edit, Trash2, Plus, ChevronLeft, ChevronRight, ChevronsLeft, ChevronsRight } from "lucide-react";
...
export default function MedicationDatabasePage() {
  ...
  const fetchDatabase = async () => {
    setIsLoading(true);
    // Jalankan fetch role dan data secara paralel agar lebih cepat
    ...
    else toast.error(medsRes.error || "Gagal memuat database obat.");
  ...
  const handleDeleteConfirm = async () => {
    ...
    if (res.success) {
      ...
      toast.error(res.error || "Gagal menghapus obat.");
    }
    setIsDeleting(false);
    setMedToDelete(null);
  };
  ...
  return (
    <div className="w-full min-h-screen p-4 sm:p-6 md:p-8 lg:p-10 transition-colors bg-slate-50 dark:bg-slate-950">
      ...
      {/* CUSTOM DELETE MODAL (HANYA UNTUK SUPER ADMIN) */}
      {medToDelete && (
        <div className="fixed inset-0 z-[9999] bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in duration-200">
          <div className="bg-white dark:bg-slate-900 rounded-3xl p-8 max-w-sm w-full text-center shadow-2xl animate-in zoom-in-95">
            ...
            <h3 className="text-xl font-black mb-2 text-slate-800 dark:text-white uppercase tracking-tight">Hapus Database?</h3>
            <p className="text-sm text-slate-500 mb-6 font-medium leading-relaxed">
              Anda yakin ingin menghapus <strong>{lang === 'id' ? medToDelete.name_id : medToDelete.name_en}</strong> secara permanen? Tindakan ini tidak dapat dibatalkan.
            </p>
            <div className="flex flex-col gap-3">
              <Button onClick={handleDeleteConfirm} disabled={isDeleting} className="w-full h-12 bg-red-600 hover:bg-red-500 text-white font-black uppercase rounded-xl">
                {isDeleting ? <Loader2 className="w-5 h-5 animate-spin mx-auto" /> : "YA, HAPUS PERMANEN"}
              </Button>
              <Button variant="ghost" onClick={() => setMedToDelete(null)} disabled={isDeleting} className="w-full h-12 font-bold uppercase rounded-xl border border-slate-200 dark:border-slate-700">Batal</Button>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="max-w-[1400px] mx-auto space-y-8 pb-10">
        ...
        <div className="bg-white dark:bg-slate-900 p-6 md:p-8 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm relative overflow-hidden transition-colors flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
          ...
          <div className="relative z-10 w-full lg:w-auto">
            ...
            {/* TOMBOL TAMBAH OBAT KHUSUS ADMIN (INLINE) */}
            {isAdmin && (
              <Button onClick={() => toast("Fitur Tambah Obat Segera Hadir!", { icon: "🛠️"})} className="mt-5 h-11 bg-slate-800 hover:bg-slate-700 dark:bg-sky-600 dark:hover:bg-sky-500 text-white font-bold px-5 rounded-xl shadow-md uppercase tracking-wider text-xs flex items-center gap-2 transition-all active:scale-[0.98]">
                <Plus className="w-4 h-4 font-black" /> {lang === 'id' ? "Tambah Obat Baru (Admin)" : "Add New Medication"}
              </Button>
            )}
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
              {currentData.map((med, index) => (
                <div key={med.id} className="relative bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 overflow-hidden shadow-sm hover:shadow-[0_0_20px_rgba(14,165,233,0.25)] dark:hover:shadow-[0_0_20px_rgba(14,165,233,0.15)] transition-all duration-300 hover:border-sky-300 dark:hover:border-sky-700 flex flex-col group">
                  ...
                  {/* TOMBOL EDIT & DELETE (HANYA MUNCUL JIKA ADMIN/SUPER ADMIN) */}
                  {isAdmin && (
                    <div className="absolute top-4 right-4 z-30 flex items-center gap-2 opacity-100 md:opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                      <button onClick={() => toast("Fitur Edit Segera Hadir!", { icon: "📝"})} className="p-2 bg-white/90 dark:bg-slate-800/90 hover:bg-sky-100 dark:hover:bg-sky-900/50 text-sky-600 dark:text-sky-400 backdrop-blur rounded-full shadow-md border border-slate-200 dark:border-slate-700 transition-colors" title="Edit">
                        <Edit className="w-4 h-4" />
                      </button>
                      {isSuperAdmin && (
                        <button onClick={() => setMedToDelete(med)} className="p-2 bg-white/90 dark:bg-slate-800/90 hover:bg-red-100 dark:hover:bg-red-900/50 text-red-600 dark:text-red-400 backdrop-blur rounded-full shadow-md border border-slate-200 dark:border-slate-700 transition-colors" title="Delete (Super Admin)">
                  ...
                  {/* HEADER KARTU OBAT: FLEXBOX ANTI TABRAK */}
                  <div className="p-5 sm:p-6 border-b border-slate-100 dark:border-slate-800/80 bg-slate-50/50 dark:bg-slate-950/50 relative overflow-hidden flex items-start gap-4">
                    ...
                    {/* NOMOR URUT: Bebas Berdiri Sendiri */}
                    ...
                    {/* TEKS JUDUL: Bebas Turun ke Bawah Tanpa Dipotong */}
                    <div className="flex-1 relative z-10 min-w-0 pr-8 md:pr-14"> {/* Pr-14 memberi ruang untuk tombol Admin */}
