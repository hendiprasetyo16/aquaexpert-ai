...
// 💡 STATE BARU UNTUK MENAMPUNG PDF YANG SUDAH DIKONVERSI
  const [pdfData, setPdfData] = useState<UniversalPDFData | null>(null);

  // 💡 FUNGSI AJAIB: MENGUBAH WEBP MENJADI JPEG MENGGUNAKAN CANVAS
  const convertToJpegBase64 = (url: string): Promise<string | undefined> => {
    return new Promise((resolve) => {
      const img = new window.Image();
      img.crossOrigin = "anonymous"; // Hindari blokir CORS Supabase
      img.onload = () => {
        const canvas = document.createElement("canvas");
        ...
        // Beri background putih agar gambar transparan tidak menjadi hitam
        ctx.fillStyle = "#ffffff"; 
        ctx.fillRect(0, 0, canvas.width, canvas.height);
        ctx.drawImage(img, 0, 0);
        resolve(canvas.toDataURL("image/jpeg", 0.8)); // Convert ke JPG!
      };
      img.onerror = () => resolve(undefined); // Jika error, lewati gambar
      img.src = url;
    });
  };

  // 💡 ASYNC EFFECT: Membangun data PDF di background saat 'results' berubah
  useEffect(() => {
    const buildPdfData = async () => {
      ...
      setPdfData(null); // Reset sementara proses konversi berjalan

      const mappedResults = await Promise.all(results.map(async (algae) => {
        ...
        const preventionKeywords = language === 'id' 
          ? ["jaga", "rutin", "hindari", "jangan", "pastikan", "cegah", "kuras", "bersihkan", "kontrol"] 
          : ["maintain", "routine", "avoid", "do not", "ensure", "prevent", "clean", "regular", "control"];
        
        const treatments: string[] = [];
        const preventions: string[] = [];
        allSolutions.forEach(s => {
          if (preventionKeywords.some(kw => s.toLowerCase().includes(kw))) preventions.push(s);
          else treatments.push(s);
        });
        if (preventions.length === 0 && treatments.length > 0) preventions.push(treatments.pop()!);

        // Tunggu gambar diubah ke JPG
        let finalImageUrl = undefined;
        if (algae.image_url) {
          finalImageUrl = await convertToJpegBase64(algae.image_url);
        }

        return {
          name: language === 'en' && algae.name_en ? algae.name_en : algae.name_id,
          imageUrl: finalImageUrl, 
          scoreText: `${algae.matchScore} ${language === 'id' ? 'Poin' : 'Pts'} - ${getConfidenceLabel(algae.matchConfidenceKey)}`,
          reasons: algae.matchReasons,
          causes: topCauses,
          treatments: treatments,
          preventions: preventions,
          // 💡 UBAH BARIS INI: Gunakan getAlgaeTagDesc agar dampaknya tidak berupa kode 'stunted_growth' tapi bahasa yang benar
          impacts: algae.affected_conditions ? algae.affected_conditions.map(c => getAlgaeTagDesc(c, language)) : []
        };
      }));
      ...
      setPdfData({
        reportTitle: isId ? "Laporan Analisis Pakar Alga" : "Algae Expert Analysis Report",
        date: isId ? `Dicetak pada: ${timestamp}` : `Printed on: ${timestamp}`,
        dict: {
          subBrand: isId ? "Sistem Pakar & Analisis Ekosistem Akuarium" : "Aquarium Ecosystem Analysis & Expert System",
          analysis: isId ? "Laporan Analisis" : "Analysis Report",
          impacts: isId ? "Dampak Kerusakan Ekosistem" : "Ecosystem Damages", // 💡 TAMBAHKAN BARIS KAMUS INI
          causes: isId ? "Kemungkinan Penyebab" : "Root Causes",
          treatment: isId ? "Langkah Penanganan" : "Treatment Plan",
          prevention: isId ? "Tindakan Pencegahan" : "Prevention",
    ...
    buildPdfData();
  }, [results, language, dictRoot]); // Effect berjalan ulang jika bahasa/hasil diubah
              ...
              <div className="animate-in fade-in duration-500 slide-in-from-bottom-4">
                <div className="mb-6 flex flex-col sm:flex-row sm:items-end justify-between border-b border-slate-200 dark:border-slate-800 pb-5">
                  ...
                  <div className="mt-4 sm:mt-0 flex items-center gap-3">
                    {pdfData ? (
                      <PDFDownloadLink
                        document={<GlobalExpertPDFReport data={pdfData} />}
                        fileName={`AquaExpert_Algae_Report_${new Date().toISOString().split('T')[0]}.pdf`}
                      >
                        {({ loading: isPdfLoading }) => (
                          <Button disabled={isPdfLoading} variant="outline" className="border-teal-200 text-teal-700 hover:bg-teal-50 dark:border-teal-900/50 dark:text-teal-400 font-bold rounded-full shadow-sm">
                            <FileDown className="w-4 h-4 mr-2" />
                            {isPdfLoading ? (language === 'id' ? "Menyiapkan..." : "Preparing...") : (language === 'id' ? "Unduh PDF" : "Download PDF")}
                          </Button>
                        )}
                      </PDFDownloadLink>
                    ) : (
                      <Button disabled variant="outline" className="border-slate-200 text-slate-500 font-bold rounded-full shadow-sm opacity-70">
                        <Loader2 className="w-4 h-4 mr-2 animate-spin" />
                        {language === 'id' ? "Memproses PDF..." : "Processing PDF..."}
                      </Button>
                    )}
                           
                    <span className="inline-flex items-center gap-2 bg-teal-50 dark:bg-teal-950/50 text-teal-700 dark:text-teal-400 text-sm md:text-base px-5 py-2 rounded-full border border-teal-200 dark:border-teal-900 font-bold whitespace-nowrap shadow-sm">
