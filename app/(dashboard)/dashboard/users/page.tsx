  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-6 pb-10 space-y-6 text-slate-900 dark:text-slate-100">
        ...
        <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
          <Card className="border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/60 shadow-sm transition-colors duration-300">
            <CardContent className="flex items-center gap-4 p-4">
              ...
              <div><p className="text-sm font-medium text-slate-500 dark:text-slate-400">Total Akun</p><p className="text-2xl font-bold text-gray-900 dark:text-slate-100">{stats.total}</p></div>
            </CardContent>
          </Card>
          ...
          {currentUserRole === "super_admin" && (
            <Card className="border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/60 shadow-sm transition-colors duration-300">
              <CardContent className="flex items-center gap-4 p-4">
                ...
                <div><p className="text-sm font-medium text-slate-500 dark:text-slate-400">Super Admin</p><p className="text-2xl font-bold text-gray-900 dark:text-slate-100">{stats.superAdmin}</p></div>
              </CardContent>
            </Card>
          )}
          ...
          {currentUserRole === "super_admin" && (
            <Card className="border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/60 shadow-sm transition-colors duration-300">
              <CardContent className="flex items-center gap-4 p-4">
                ...
                <div><p className="text-sm font-medium text-slate-500 dark:text-slate-400">Admin</p><p className="text-2xl font-bold text-gray-900 dark:text-slate-100">{stats.admin}</p></div>
              </CardContent>
            </Card>
          )}
          ...
          <Card className="border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/60 shadow-sm transition-colors duration-300">
            <CardContent className="flex items-center gap-4 p-4">
              ...
              <div><p className="text-sm font-medium text-slate-500 dark:text-slate-400">User</p><p className="text-2xl font-bold text-gray-900 dark:text-slate-100">{stats.user}</p></div>
            </CardContent>
          </Card>
        </div>
      )}
      ...
      {!loading && (
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
          <div className="relative flex-1">
            ...
            <input type="text" placeholder="Cari nama atau email pengguna..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full rounded-md border border-slate-300 dark:border-slate-800 bg-white dark:bg-slate-900 py-2 pl-10 pr-4 text-sm text-slate-900 dark:text-slate-200 outline-none focus:border-teal-500 transition-colors" />
          </div>
          
          <div className="relative w-full sm:w-40">
            <Filter className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" />
            <select value={roleFilter} onChange={(e) => setRoleFilter(e.target.value as any)} className="w-full appearance-none rounded-md border border-slate-300 dark:border-slate-800 bg-white dark:bg-slate-900 py-2 py-2 pl-10 pr-4 text-sm text-slate-900 dark:text-slate-200 outline-none focus:border-teal-500 transition-colors">
              ...
              {currentUserRole === "super_admin" && (
                <><option value="admin" className="bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-200">Admin</option><option value="super_admin" className="bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-200">Super Admin</option></>
              )}
            </select>
          </div>
      ...
      {loading ? (
        ...
        <div className="flex h-40 flex-col items-center justify-center text-slate-500 dark:text-slate-400"><Search className="mb-2 h-8 w-8 opacity-40" /><p>Tidak ada pengguna yang cocok.</p></div>
      ) : (
                  ...
                  <div className="flex gap-3 overflow-hidden w-full">
                    ...
                    <div className="overflow-hidden w-full">
                      ...
                      <div className="mt-2 flex flex-col gap-1.5 text-xs text-slate-600 dark:text-slate-400">
                        ...
                        {user.created_at && (
                          <div className="flex items-center gap-2"><CalendarDays className="h-3.5 w-3.5 shrink-0 text-slate-400 dark:text-slate-500" /><span className="truncate">Terdaftar: {new Date(user.created_at).toLocaleString('id-ID', { day: 'numeric', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' })}</span></div>
                        )}
                        <div className="flex items-center gap-2">
                          <Activity className="h-3.5 w-3.5 shrink-0 text-slate-400 dark:text-slate-500" />
                          {user.last_login_at ? (
                            <span className="truncate text-teal-600 dark:text-teal-400 font-medium">Login: {new Date(user.last_login_at).toLocaleString('id-ID', { day: 'numeric', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' })}</span>
                          ) : (
                            <span className="truncate text-yellow-600 dark:text-yellow-500 font-medium italic">Belum pernah login</span>
                          )}
                        </div>
                      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/70 dark:bg-slate-950/90 p-4 backdrop-blur-sm overflow-y-auto">
          <div className="w-full max-w-sm rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-6 shadow-xl transition-all">
            ...
            <p className="text-sm text-slate-600 dark:text-slate-400 mb-6 leading-relaxed">
              Anda yakin ingin <strong>{userToToggle.is_active ? "memblokir" : "mengaktifkan"}</strong> akses login untuk <strong className="text-gray-900 dark:text-slate-200">{userToToggle.full_name}</strong>?
            </p>
            <div className="flex justify-end gap-3 border-t border-slate-200 dark:border-slate-800 pt-4">
              <button 
                ...
                className="rounded-md px-4 py-2 text-sm font-medium text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors"
              >
                Batal
              </button>
              <button 
                ...
                className={`rounded-md px-4 py-2 text-sm font-medium text-white transition-colors disabled:opacity-50 ${userToToggle.is_active ? 'bg-red-600 hover:bg-red-500' : 'bg-teal-600 hover:bg-teal-500'}`}
              >
                {isSubmitting ? "Memproses..." : (userToToggle.is_active ? "Ya, Blokir" : "Ya, Aktifkan")}
              </button>
            </div>
          </div>
        </div>
          ...
          <div className="w-full max-w-sm rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-6 shadow-xl transition-all">
            ...
            <div className="flex justify-end gap-3 border-t border-slate-200 dark:border-slate-800 pt-4">
              ...
              <button 
                ...
                className="rounded-md px-4 py-2 text-sm font-medium text-white bg-blue-600 hover:bg-blue-500 transition-colors disabled:opacity-50"
              >
