...
export async function createFishAction(fishData: Partial<Fish>) {
  try {
    ...
    if (existingFish) throw new Error(`Ikan "${fishData.name_id}" sudah ada.`);
    ...
    await pushNotificationAction(
      "Data Ikan Baru Ditambahkan",
      `Menambahkan spesies ikan: ${fishData.name_id}.`,
      "data_crud",
      user.fullName
    );
...
export async function updateFishAction(id: string, fishData: Partial<Fish>) {
  try {
    ...
    if (duplicateFish) throw new Error(`Ikan "${fishData.name_id}" sudah ada.`);
    ...
    // 💡 NOTIFIKASI
    await pushNotificationAction(
      "Data Ikan Diperbarui",
      `Memperbarui spesies ikan: ${fishData.name_id || data?.name_id}.`,
      "data_crud",
      user.fullName
    );
...
export async function hardDeleteFishAction(id: string) {
  try {
    ...
    const { data: fish, error: fetchError } = await supabase.from("fishes").select("name_id, image_url, gallery_urls").eq("id", id).single();
    ...
    // 💡 NOTIFIKASI
    await pushNotificationAction(
      "Data Ikan Dihapus Permanen",
      `Menghapus permanen ikan: ${fish?.name_id} beserta gambarnya.`,
      "data_crud",
      user.fullName
    );
...
export async function restoreFishAction(id: string) {
  try {
    ...
    const { error } = await supabase.from("fishes").update({ is_active: true, updated_by: user.id }).eq("id", id);
    if (error) throw new Error(error.message);

    // 💡 NOTIFIKASI
    await pushNotificationAction(
      "Data Ikan Dipulihkan",
      `Memulihkan ikan ${archivedFish.name_id} dari arsip.`,
      "data_crud",
      user.fullName
    );
