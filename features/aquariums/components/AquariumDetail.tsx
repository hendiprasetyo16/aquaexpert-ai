...
export default function AquariumDetail() {
  ...
  if (loading && !aquarium) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh]">
        ...
        <p className="text-slate-500 font-medium animate-pulse">Memuat data ekosistem...</p>
      </div>
    );
  }
  ...
  return (
    <div className="w-full pb-20 animate-in fade-in duration-500">
      ...
      <div className="relative w-full h-[35vh] sm:h-[45vh] bg-slate-900 overflow-hidden">
        {aquarium.image_url && !imgError ? (
          <Image 
            ...
            alt={aquarium.name} 
            fill 
            className={`object-cover ${isArchived ? 'opacity-40 grayscale' : 'opacity-60'}`}
          ...
          <div className="max-w-6xl mx-auto flex flex-col md:flex-row md:items-end justify-between gap-4">
            ...
            <div className="flex flex-wrap items-center gap-2 shrink-0 mt-4 md:mt-0">
              ...
              <Button 
                ...
                className={isArchived ? "bg-emerald-500/80 hover:bg-emerald-500 text-white border-none backdrop-blur-md" : "bg-amber-500/80 hover:bg-amber-500 text-white border-none backdrop-blur-md"}
              >
      ...
      {/* 2. KONTEN UTAMA */}
      <div className="max-w-6xl mx-auto p-4 sm:p-8 -mt-6 relative z-20">
        
        {/* TAB NAVBAR */}
        <div className="flex overflow-x-auto bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 mb-6 p-1 no-scrollbar">
          ...
          <div className="space-y-6">
            <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800">
              ...
              <div className="space-y-4">
                <div>
                  <p className="text-xs text-slate-400 mb-1">{lang === 'id' ? "Umur Sistem" : "System Age"}</p>
                ...
                <div>
                  <p className="text-xs text-slate-400 mb-1">{lang === 'id' ? "Dimensi" : "Dimensions"} (P x L x T)</p>
                ...
                <hr className="border-slate-100 dark:border-slate-800" />
                <div>
                  <p className="text-xs text-slate-400 mb-1">{lang === 'id' ? "Substrat Dasar" : "Substrate"}</p>
          ...
          <div className="md:col-span-2 space-y-6">
            <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className="p-4 rounded-xl bg-slate-50 dark:bg-slate-950/50 border border-slate-100 dark:border-slate-800">
                  <p className="text-xs text-slate-500 mb-1">{lang === 'id' ? "Lampu" : "Light"}</p>
                  <p className="font-bold text-slate-800 dark:text-slate-200">{getLightDesc(aquarium.light_type, lang)}</p>
                  <p className="text-xs text-slate-400 mt-1">
                    {aquarium.light_wattage ? `${aquarium.light_wattage}W • ` : ''} {aquarium.photoperiod_hours ? `${aquarium.photoperiod_hours} ${lang === 'id' ? "Jam/Hari" : "Hours/Day"}` : ''}
                  </p>
                </div>
                <div className="p-4 rounded-xl bg-slate-50 dark:bg-slate-950/50 border border-slate-100 dark:border-slate-800">
                ...
                <div className="p-4 rounded-xl bg-slate-50 dark:bg-slate-950/50 border border-slate-100 dark:border-slate-800">
                  <p className="text-xs text-slate-500 mb-1">Heater</p>
                  <p className="font-bold text-slate-800 dark:text-slate-200">
                    {aquarium.heater_enabled ? (lang === 'id' ? "Gunakan Heater" : "Heater Enabled") : (lang === 'id' ? "Tanpa Heater" : "No Heater")}
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="p-4 rounded-xl bg-blue-50 dark:bg-blue-950/20 border border-blue-100 dark:border-blue-900/30">
                  <p className="text-xs text-blue-600/70 dark:text-blue-400/70 mb-1 font-bold uppercase">{lang === 'id' ? "Ganti Air" : "Water Change"}</p>
                  <p className="font-bold text-blue-900 dark:text-blue-200 text-lg">
                    {aquarium.water_change_percent}% / {aquarium.water_change_interval_days} {lang === 'id' ? "Hari" : "Days"}
                  </p>
                </div>
                <div className="p-4 rounded-xl bg-emerald-50 dark:bg-emerald-950/20 border border-emerald-100 dark:border-emerald-900/30">
                  <p className="text-xs text-emerald-600/70 dark:text-emerald-400/70 mb-1 font-bold uppercase">{lang === 'id' ? "Sistem Pupuk" : "Fertilizer System"}</p>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm animate-in fade-in">
          <div className={`w-full max-w-sm rounded-xl bg-white dark:bg-slate-900 p-6 shadow-2xl border ${isArchived ? 'border-emerald-200 dark:border-emerald-900/50' : 'border-amber-200 dark:border-amber-900/50'}`}>
            <h3 className="text-xl font-bold text-gray-900 dark:text-slate-100 mb-2">
              {isArchived ? (lang === 'id' ? "Pulihkan Akuarium?" : "Restore Aquarium?") : (lang === 'id' ? "Arsipkan Akuarium?" : "Archive Aquarium?")}
            </h3>
            ...
            <div className="flex justify-end gap-3">
              <Button variant="outline" onClick={() => setShowArchiveModal(false)} disabled={loading}>{lang === 'id' ? "Batal" : "Cancel"}</Button>
              <Button onClick={handleArchiveToggle} disabled={loading} className={isArchived ? "bg-emerald-600 hover:bg-emerald-500 text-white" : "bg-amber-600 hover:bg-amber-500 text-white"}>
                {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : (isArchived ? detailDict.restore : detailDict.archive)}
              </Button>
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm animate-in fade-in">
          <div className="w-full max-w-sm rounded-xl bg-white dark:bg-slate-900 p-6 shadow-2xl border border-red-200 dark:border-red-900/50">
            <div className="flex items-center gap-3 mb-2">
              ...
              <h3 className="text-xl font-bold text-red-600">{lang === 'id' ? "Hapus Permanen?" : "Delete Permanently?"}</h3>
            </div>
            <p className="text-sm text-slate-600 dark:text-slate-400 mb-6">
            ...
            <div className="flex justify-end gap-3">
              ...
              <Button onClick={handleDelete} disabled={loading} variant="destructive">
                {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : (lang === 'id' ? "Ya, Hapus Total" : "Yes, Delete All")}
              </Button>
            </div>
          </div>
        </div>
