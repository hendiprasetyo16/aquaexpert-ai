...
import { Activity, Fish, AlertCircle, HeartPulse, Loader2, Syringe } from "lucide-react";
import { Button } from "@/components/ui/button";
import { getActiveTreatmentsAction, ActiveTreatmentDto } from "@/features/diseases/actions/start-treatment.actions"; // FIX: Hapus .ts
...
export default function TreatmentWardPage() {
  ...
  return (
    <div className="w-full min-h-screen p-4 sm:p-6 md:p-8 lg:p-10 transition-colors bg-slate-50 dark:bg-slate-950">
      <div className="max-w-[1400px] mx-auto space-y-8 pb-10">
          ...
          <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div>
              <h1 className="text-3xl md:text-4xl font-extrabold text-rose-600 dark:text-rose-500 flex items-center gap-3">
                ...
                {tDict.title || "Ruang Rawat Inap"}
              </h1>
              <p className="mt-3 text-slate-600 dark:text-slate-400 max-w-3xl text-sm md:text-base font-medium">
                {tDict.subtitle || "Pantau dan catat progres harian pasien fauna Anda secara global."}
              </p>
            </div>
          </div>
        </div>
        ...
        {isLoading ? (
          <div className="flex flex-col items-center justify-center p-20 text-slate-500">
            ...
            <p className="font-bold animate-pulse">Memuat data pasien...</p>
          </div>
        ) : treatments.length === 0 ? (
          <div className="bg-white dark:bg-slate-900 rounded-3xl border-2 border-dashed border-slate-200 dark:border-slate-800 p-16 flex flex-col items-center justify-center text-center shadow-sm">
            ...
            <h3 className="text-xl font-black text-slate-700 dark:text-slate-200 mb-2">{tDict.emptyTitle || "Tidak Ada Pasien"}</h3>
            <p className="text-slate-500 dark:text-slate-400 font-medium">
              {tDict.emptyDesc || "Saat ini tidak ada fauna yang sedang dalam masa pengobatan aktif."}
            </p>
          </div>
        ) : (
                ...
                <div key={session.id} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl overflow-hidden shadow-sm hover:shadow-md transition-all flex flex-col">
                  <div className="p-5 border-b border-slate-100 dark:border-slate-800/60 bg-slate-50/50 dark:bg-slate-950/50 flex justify-between items-start">
                    <div>
                      <div className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-rose-100 text-rose-700 dark:bg-rose-900/30 dark:text-rose-400 rounded-md text-[10px] font-black uppercase tracking-wider border border-rose-200 dark:border-rose-800/50 mb-3">
                        <AlertCircle className="w-3.5 h-3.5" /> {tDict.day || "Hari Ke-"} {dayNum}
                      </div>
                  ...
                  <div className="p-5 flex-1 flex flex-col justify-center space-y-4">
                    <div className="bg-slate-50 dark:bg-slate-950 rounded-xl p-4 border border-slate-100 dark:border-slate-800">
                      <p className="text-[10px] uppercase font-bold tracking-widest text-slate-400 mb-1">{tDict.medication || "Pengobatan"}</p>
                    ...
                    <div>
                      ...
                      <div className="w-full bg-slate-200 dark:bg-slate-800 rounded-full h-2.5 overflow-hidden">
                        <div className="bg-emerald-500 h-2.5 rounded-full" style={{ width: `${session.current_recovery_rate}%` }} />
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="p-5 pt-0">
                    ...
                    >
                      <Activity className="w-4 h-4 mr-2" /> {tDict.btnLog || "Catat Medis Hari Ini"}
                    </Button>
                  </div>
                </div>
              );
      ...
      {/* FIX 2: Menambahkan prop hasLoggedToday agar TypeScript tidak error */}
      {selectedSession && (
        <DailyLogModal 
