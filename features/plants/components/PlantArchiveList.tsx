...
export default function PlantArchiveList() {
  ...
  if (plants.length === 0) {
    return (
      // PERBAIKAN: Kotak Arsip Kosong menjadi adaptif
  ...
  return (
    <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
      {plants.map((plant) => (
        // PERBAIKAN: Card dibikin putih (terang) / gelap, dengan bayangan tipis
        <Card key={plant.id} className="border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/40 opacity-80 transition-all hover:opacity-100 shadow-sm">
          ...
          <CardContent className="border-t border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950/50 p-4 transition-colors">
            <div className="flex flex-col gap-2 sm:flex-row sm:justify-end">
              <Button 
                ...
                // PERBAIKAN: Tombol Restore (Pulihkan)
              ...
              {role === "super_admin" && (
                <Button 
                  ...
                  disabled={processingId === plant.id}
                  // PERBAIKAN: Tombol Hapus Permanen
