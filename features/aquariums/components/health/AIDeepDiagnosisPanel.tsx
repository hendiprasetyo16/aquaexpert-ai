  ...
  const RiskRadarCore = ({ currentRisk }: { currentRisk: RiskLevel }) => {
    ...
    return (
      <div className="relative flex flex-col items-center justify-center py-4 w-full">
        ...
        <div className={`relative z-10 w-20 h-20 rounded-full flex flex-col items-center justify-center bg-white dark:bg-slate-900 border-4 ${config.border} transition-all duration-700 scale-110 shadow-md`}>
    ...
    <div className="space-y-6 mt-4">
      ...
      {result && result.localDiagnosis && !loading && (
        <div className="space-y-6 animate-in fade-in duration-500">
          ...
          {/* FIX: BARIS UTAMA (KIRI: RADAR+TOMBOL, KANAN: RINGKASAN+AI) */}
          <div className="bg-white dark:bg-slate-900 p-5 sm:p-6 rounded-[2rem] border-2 border-slate-200 dark:border-slate-800 shadow-xl flex flex-col md:flex-row gap-6 items-stretch">
            
            {/* KOLOM KIRI (RADAR & DIAGNOSIS ULANG) */}
            <div className="w-full md:w-64 bg-slate-50 dark:bg-slate-950/50 p-6 rounded-[1.5rem] border border-slate-200 dark:border-slate-800 shadow-inner flex flex-col items-center justify-center shrink-0">
              <RiskRadarCore currentRisk={result.localDiagnosis.riskLevel} />
              
              <Button onClick={handleRunDiagnosis} variant="outline" className="w-full mt-6 border-indigo-200 text-indigo-600 hover:bg-indigo-50 dark:border-indigo-900/50 dark:text-indigo-400 font-bold h-11 rounded-xl shadow-sm transition-all active:scale-95">
                <RefreshCw className="w-4 h-4 mr-2" /> {lang === 'id' ? "Diagnosis Ulang" : "Re-Diagnose"}
              </Button>
            </div>

            {/* KOLOM KANAN (KETERANGAN & CATATAN PAKAR) */}
            <div className="flex-1 flex flex-col justify-start min-w-0">
              
              {/* Keterangan Singkat (Diperkecil sesuai permintaan) */}
              <div className="mb-4 px-2">
                <p className="text-slate-800 dark:text-slate-200 font-bold text-base sm:text-lg leading-relaxed">
                  {result.localDiagnosis.summary}
                </p>
              </div>

              {/* Catatan Pakar AI */}
              <div className={`relative flex-1 overflow-hidden p-5 sm:p-6 rounded-[1.5rem] border-2 shadow-sm ${result.expertAIExtras.generatedByGemini ? 'bg-gradient-to-br from-indigo-50 via-purple-50/50 to-white dark:from-indigo-950/40 dark:to-purple-950/20 border-indigo-200 dark:border-indigo-800' : 'bg-slate-50 dark:bg-slate-900 border-slate-200 dark:border-slate-800'}`}>
                {result.expertAIExtras.generatedByGemini && <Sparkles className="absolute -right-4 -top-4 w-24 h-24 text-indigo-500/10 rotate-12" />}
                
                <h4 className={`text-xs font-black uppercase tracking-widest mb-3 flex items-center gap-2 ${result.expertAIExtras.generatedByGemini ? 'text-indigo-700 dark:text-indigo-400' : 'text-slate-600 dark:text-slate-400'}`}>
                  <Bot className="w-4 h-4" /> {lang === 'id' ? "Catatan Pakar Ekologi (AI Assistant)" : "Expert Ecological Notes (AI)"}
                </h4>
                
                <div className="text-sm font-semibold leading-relaxed text-slate-800 dark:text-slate-200 space-y-3 whitespace-pre-wrap relative z-10">
