    ...
    try {
      ...
      if (data?.user?.id) {
        ...
        await supabase.from("system_activities").insert({
          title: "User Login",
          message: `User dengan email "${email}" berhasil masuk ke dalam sistem.`,
    ...
    <Card className="z-10 w-full max-w-md border-border bg-card/90 text-card-foreground backdrop-blur-sm shadow-xl transition-colors duration-300">
      ...
      <CardContent>
        ...
        <div className="mt-4 text-center text-sm text-muted-foreground">
          Belum punya akun?{" "}
          <Link href="/register" className="font-medium text-teal-600 dark:text-teal-400 hover:underline">
            Daftar di sini
          </Link>
        </div>
      </CardContent>
