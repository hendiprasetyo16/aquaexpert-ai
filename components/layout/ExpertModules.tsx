...
import { Database, Fish, Leaf, Bug, Cpu, HeartPulse, ArrowRight, BarChart, Sparkles } from "lucide-react";
  ...
  return (
    <div className="lg:col-span-2 space-y-8">
      ...
      <div className="space-y-4">
        ...
        {/* 💡 Grid-nya sekarang seragam 6 Kotak (3x2 pada layar besar, 2x3 pada layar sedang) */}
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4 sm:gap-6">
          ...
          {/* 💡 MENU BARU UNTUK MENGISI GRID (AI ASSISTANT) */}
          <div onClick={() => router.push("/dashboard/ai-assistant")} className="group cursor-pointer bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-6 rounded-3xl transition-all duration-300 ease-out relative overflow-hidden hover:-translate-y-1 hover:border-violet-400 dark:hover:border-violet-500 hover:shadow-[0_0_30px_rgba(139,92,246,0.15)] dark:hover:shadow-[0_0_40px_rgba(139,92,246,0.5)]">
            <div className="absolute -top-10 -right-10 w-32 h-32 bg-violet-500/10 dark:bg-violet-500/20 blur-3xl rounded-full group-hover:bg-violet-500/20 dark:group-hover:bg-violet-500/30 transition-colors duration-500"></div>
            <div className="relative z-10">
              <div className="w-12 h-12 rounded-xl flex items-center justify-center mb-4 bg-violet-50 dark:bg-violet-500/10 text-violet-600 dark:text-violet-400 group-hover:scale-110 transition-transform duration-300"><Sparkles className="w-6 h-6 drop-shadow-[0_0_8px_rgba(139,92,246,0.5)]" /></div>
              <h4 className="font-extrabold text-slate-800 dark:text-slate-100 text-lg mb-2 group-hover:text-violet-600 dark:group-hover:text-violet-400 transition-colors">{lang === 'id' ? "Tanya Pakar (AI)" : "AI Assistant"}</h4>
              <p className="text-xs font-medium text-slate-500 dark:text-slate-400 mb-4 line-clamp-3">{lang === 'id' ? "Asisten cerdas berbasis AI yang siap menjawab semua pertanyaan seputar ekosistem akuarium Anda." : "Smart AI assistant ready to answer all your questions about your aquarium ecosystem."}</p>
              <div className="flex items-center text-xs font-bold text-violet-600 dark:text-violet-400 uppercase tracking-wider group-hover:gap-2 transition-all gap-1">{lang === 'id' ? "Mulai Chat" : "Start Chat"} <ArrowRight className="w-4 h-4" /></div>
            </div>
          </div>
