...
export function DiseaseResultCard({ result, lang = "id", onDetailClick }: Props) {
  ...
  const getColorScheme = (score: number) => {
    if (score >= 80) return { border: "border-emerald-200 dark:border-emerald-800/50", bg: "bg-emerald-50/50 dark:bg-emerald-950/10", progress: "bg-emerald-500", text: "text-emerald-700 dark:text-emerald-400" };
    if (score >= 60) return { border: "border-amber-200 dark:border-amber-800/50", bg: "bg-amber-50/50 dark:bg-amber-950/10", progress: "bg-amber-500", text: "text-amber-700 dark:text-amber-400" };
    if (score >= 40) return { border: "border-orange-200 dark:border-orange-800/50", bg: "bg-orange-50/50 dark:bg-orange-950/10", progress: "bg-orange-500", text: "text-orange-700 dark:text-orange-400" };
    return { border: "border-rose-200 dark:border-rose-800/50", bg: "bg-rose-50/50 dark:bg-rose-950/10", progress: "bg-rose-500", text: "text-rose-700 dark:text-rose-400" };
  };
  ...
  return (
    <div className={`w-full rounded-xl border p-5 transition-all duration-200 hover:shadow-md ${style.border} ${style.bg}`}>
      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4 mb-4">
        <div>
          <h3 className="text-lg font-bold text-slate-800 dark:text-slate-100">{diseaseName}</h3>
          <p className="text-xs font-semibold uppercase tracking-wider text-slate-400 mt-1">Kategori: {disease.disease_category || "Umum"}</p>
        </div>
        <div className="flex sm:flex-col items-end gap-2 sm:gap-1 shrink-0 bg-white dark:bg-slate-950 px-4 py-2 rounded-lg border border-slate-100 dark:border-slate-800/80 shadow-sm">
          ...
          <span className={`text-2xl font-black ${style.text}`}>{confidenceScore}%</span>
        </div>
      </div>
        ...
        <div className="space-y-2 mb-4">
          ...
          {hasHallmarkMatch && (
            <div className="flex items-center gap-2.5 p-3 rounded-lg bg-emerald-500/10 text-emerald-800 dark:text-emerald-400 border border-emerald-500/20 text-xs">
              ...
              <span className="font-medium leading-relaxed">{lang === "id" ? "🎯 Tanda Mutlak Terpenuhi" : "🎯 Hallmark Match"}</span>
            </div>
          )}
          {isCritical && (
            <div className="flex items-center gap-2.5 p-3 rounded-lg bg-rose-500/10 text-rose-800 dark:text-rose-400 border border-rose-500/20 text-xs">
              ...
              <span className="font-semibold leading-relaxed">{lang === "id" ? "⚠️ Bahaya Tingkat Tinggi" : "⚠️ High Mortality Risk"}</span>
            </div>
          )}
        </div>
      )}
      ...
      <div className="flex justify-end pt-2">
        <button onClick={() => onDetailClick(disease.id)} className="flex items-center gap-1 px-4 py-2 rounded-lg text-xs font-bold text-slate-700 hover:text-blue-600 dark:text-slate-300 dark:hover:text-blue-400 transition-colors group">
