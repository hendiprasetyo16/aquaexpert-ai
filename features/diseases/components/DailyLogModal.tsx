...
import { X, Save, Loader2, Syringe, Droplets, Eye, ArrowRightLeft, Info, Trash2, AlertTriangle, Activity } from "lucide-react";
...
export default function DailyLogModal({ session, isOpen, onClose, onSuccess, tDict, lang, hasLoggedToday }: Props) {
  ...
  return (
    <>
      {/* 
        FIX 1: MODAL HAPUS DIKELUARKAN DARI DALAM CONTAINER OVERFLOW
        Sekarang modal ini memiliki z-index 999 dan membungkus seluruh layar dengan sempurna 
      */}
      {showDeleteConfirm && (
        <div className="fixed inset-0 z-[999] bg-slate-900/90 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in duration-200">
          <div className="bg-white dark:bg-slate-900 rounded-3xl p-8 max-w-sm w-full text-center shadow-2xl animate-in zoom-in-95">
            ...
            <p className="text-sm font-medium text-slate-600 dark:text-slate-400 mb-6 leading-relaxed">
              {lang === 'id' 
                ? "PERINGATAN: Gunakan ini HANYA JIKA Anda salah memasukkan data. Semua rekam medis pada sesi ini akan hangus." 
                : "WARNING: Use ONLY IF you made a data entry mistake. All logs will be destroyed."}
            </p>
            <div className="flex gap-3">
              <Button type="button" variant="outline" onClick={() => setShowDeleteConfirm(false)} disabled={isDeleting} className="flex-1 h-12 rounded-xl font-bold uppercase text-slate-600">Batal</Button>
      ...
      <div className="fixed inset-0 z-[100] flex items-start justify-center overflow-y-auto bg-slate-900/80 dark:bg-black/80 p-4 backdrop-blur-sm animate-in fade-in duration-200">
        <div className="w-full max-w-2xl bg-white dark:bg-slate-950 rounded-3xl shadow-2xl flex flex-col overflow-hidden max-h-[90vh] md:max-h-[85vh] border border-slate-200 dark:border-slate-800 my-auto">
          ...
          <div className="p-5 sm:p-6 border-b border-slate-100 dark:border-slate-800 flex justify-between items-start bg-slate-50 dark:bg-slate-900 shrink-0">
            ...
            <button onClick={onClose} className="p-2 bg-slate-200 text-slate-500 hover:bg-rose-500 hover:text-white rounded-full transition-colors"><X className="w-5 h-5"/></button>
          </div>
          ...
          <form id="daily-log-form" onSubmit={handleSubmit} className="flex-1 flex flex-col overflow-hidden">
            <div className="flex-1 overflow-y-auto custom-scrollbar p-5 sm:p-6 space-y-6 bg-white dark:bg-slate-950">
              ...
              {hasLoggedToday && (
                <div className="bg-blue-50 border border-blue-200 p-4 rounded-xl flex gap-3">
                  ...
                  <p className="text-[11px] sm:text-xs font-medium text-blue-700 leading-relaxed">
                    Sistem membatasi 1 log per hari. Anda SUDAH MENGISI log hari ini. Menyimpan form ini akan MENGUPDATE (menimpa) data Anda sebelumnya.
                  </p>
                </div>
              )}

              {/* FIX 2: WARNA PROYEKSI KESEMBUHAN DISESUAIKAN UNTUK MODE TERANG & GELAP */}
                 ...
                 <div className="bg-white dark:bg-slate-800 p-3 rounded-lg border border-slate-200 dark:border-slate-700">
                   <p className="text-[11px] text-slate-600 dark:text-slate-400 font-medium leading-snug">
                     {actionTaken === "Medication Changed" 
                       ? "⚠️ PERINGATAN: Memilih 'Ganti Obat' akan menghentikan sesi ini dan memindahkannya ke Riwayat. Anda harus membuka sesi baru setelah ini."
                       : projectedRecovery === 100 
                       ? "🎉 LUAR BIASA! Gejala habis. Jika disimpan, sistem akan menyatakan ikan SEMBUH TOTAL."
                       : "Cabut centang gejala di bawah untuk melihat persentase ini naik secara real-time."
                     }
                   </p>
                 </div>
              </div>
              ...
              <div className="space-y-2 pt-2">
                <div className="flex justify-between items-center mb-2">
                  <label className="text-[11px] font-black text-slate-500 uppercase tracking-widest">Tindakan Utama Hari Ini</label>
                  <Button type="button" onClick={() => setShowDeleteConfirm(true)} variant="ghost" className="h-8 px-2 text-[10px] uppercase font-black text-red-500 hover:bg-red-50 hover:text-red-600">
                    <Trash2 className="w-3.5 h-3.5 mr-1" /> Hapus Sesi (Salah Input)
                  </Button>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {actionOptions.map(opt => (
                    <div key={opt.val} onClick={() => setActionTaken(opt.val)} className={`cursor-pointer flex flex-col gap-1 p-3 rounded-xl border-2 transition-all ${actionTaken === opt.val ? 'border-rose-500 bg-rose-50 dark:bg-rose-900/20' : 'border-slate-200 hover:border-slate-300 dark:border-slate-800'}`}>
                      ...
                      <p className={`text-[10px] ${actionTaken === opt.val ? 'text-rose-600 dark:text-rose-500' : 'text-slate-500'}`}>{opt.desc}</p>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-2">
                  <label className="text-[11px] font-black text-slate-500 uppercase tracking-widest">Dosis Diberikan ({session.medication?.dosage_unit})</label>
                  <Input type="number" min={0} step={0.1} value={medicationDose} onChange={(e) => setMedicationDose(e.target.value ? Number(e.target.value) : "")} disabled={actionTaken === "Observed" || actionTaken === "Medication Changed"} className="h-12 rounded-xl bg-slate-50 dark:bg-slate-900 border-slate-200 dark:border-slate-800 font-bold" />
                </div>
                <div className="space-y-2">
                  <label className="text-[11px] font-black text-rose-500 uppercase tracking-widest">Kematian Baru (Ekor)</label>
                  <Input type="number" min={0} value={newFishLostCount} onChange={(e) => setNewFishLostCount(e.target.value ? Number(e.target.value) : "")} className="h-12 rounded-xl bg-rose-50 dark:bg-rose-950/50 border-rose-200 dark:border-rose-900 font-bold text-rose-600" />
                </div>
              </div>

              <div className="space-y-3 p-4 bg-slate-50 dark:bg-slate-900/50 rounded-xl border border-slate-200 dark:border-slate-800">
                <label className="text-[11px] font-black text-slate-500 uppercase tracking-widest flex justify-between">Checklist Sisa Gejala</label>
                <div className="space-y-2">
            ...
            <div className="p-5 sm:p-6 border-t border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-900 flex gap-3 shrink-0">
               <Button type="button" variant="outline" onClick={onClose} disabled={isSubmitting} className="flex-1 h-12 sm:h-14 rounded-xl font-bold uppercase border-slate-200 text-slate-600">Batal</Button>
               <Button type="submit" form="daily-log-form" disabled={isSubmitting} className="flex-1 h-12 sm:h-14 bg-rose-600 hover:bg-rose-500 text-white font-black text-xs uppercase rounded-xl shadow-[0_0_15px_rgba(225,29,72,0.3)]">
                 {isSubmitting ? <Loader2 className="w-5 h-5 animate-spin mx-auto" /> : <><Save className="w-4 h-4 mr-1.5" /> SIMPAN DATA</>}
               </Button>
            </div>
          </form>
